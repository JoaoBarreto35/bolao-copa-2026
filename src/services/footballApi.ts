 ...
 * - Busca a temporada e os dias dos jogos como fontes secundárias.
 * - Busca Brasil x Haiti e Escócia x Brasil pelo ID exato do evento.
 * - Os eventos buscados por ID entram por último e têm prioridade.
 * - Se a API falhar, o app mantém o fallback local.
 */
const SPORTS_DB_BASE_URL = 'https://www.thesportsdb.com/api/v1/json';

const SPORTS_DB_KEY =
  process.env.REACT_APP_THESPORTSDB_API_KEY ?? '123';

const WORLD_CUP_LEAGUE_ID =
  process.env.REACT_APP_THESPORTSDB_WORLD_CUP_LEAGUE_ID ?? '4429';

const SEASON =
  process.env.REACT_APP_WORLD_CUP_SEASON ?? '2026';

const BRAZIL_HAITI_EVENT_ID =
  process.env.REACT_APP_THESPORTSDB_BRAZIL_HAITI_EVENT_ID ?? '2391748';

const SCOTLAND_BRAZIL_EVENT_ID =
  process.env.REACT_APP_THESPORTSDB_SCOTLAND_BRAZIL_EVENT_ID ?? '2391765';
